            ...
            <div class="content-page">
                <div class="content">
                    ...
                    <div class="container-fluid">
                        ...
                        <!-- start page title -->
                        <a href="#" id="emergencyAccessBtn" class="btn btn-danger">Emergency Access</a>
                        <div id="patientDetailsTable"></div>
                        <script src="https://code.jquery.com/jquery-3.6.4.min.js"></script>
<script>
    $(document).ready(function () {
        // Handle emergency access button click
        $("#emergencyAccessBtn").click(function (e) {
            e.preventDefault();

            // Assuming you have a server-side PHP file to fetch patient details
            $.ajax({
                ...
                success: function (response) {
                    if (response.success) {
                        // Display all patient details in a table format
                        displayAllPatientDetails(response.patients);
                    } else {
                        alert('Failed to fetch patient details. Please try again.');
                    }
                },
                error: function () {
                    alert('Error in fetching patient details. Please try again.');
                }
            });
        });

        function displayAllPatientDetails(patients) {
            // Create a table and append it to the specified section
            ...
            for (var i = 0; i < patients.length; i++) {
                tableHtml += '<tr>' +
                                '<td>' + patients[i].pat_fname + ' ' + patients[i].pat_lname + '</td>' +
                                '<td>' + patients[i].pat_ailment + '</td>' +
                             '</tr>';
            }

            tableHtml += '</tbody></table>';

            // Empty the previous content and append the new table
            $('#patientDetailsTable').empty().append(tableHtml);
        }
    });
</script>
</body>
